...
import { useConfirm, useSnackbar } from "src/app/components/notification";
import NumberInput from "src/app/components/number-input/NumberInput";
...
const CheckoutPage = () => {
	...
	const checkOutSuccessInfo = useAppSelector(
		(state) => state.cart.checkOutSuccessInfo
	);
	...
	React.useEffect(() => {
		if (checkOutSuccessInfo) {
			snackbar.success({
				content:
					"Cảm ơn bạn đã mua hàng. Mã hóa đơn của bạn là " +
	...
	const handleRemoveSelectedItems = () => {
		...
		confirm({
			title: "Xác nhận",
			content: "Bạn có muốn xóa các mục đã chọn không?",
		}).then(() =>
			dispatch(
				removeCartItems(
					cartItems
	...
	const handleRemoveItem = (itemIndex: number) => {
		confirm({
			title: "Xác nhận",
			content: "Bạn có muốn xóa mục đã chọn không?",
		}).then(() =>
			dispatch(
				updateCartItem({
					variantId: cartItems[itemIndex].variantId,
												...
												<MemoCartDetailRow
													...
													iconCheckAll={
														<Checkbox
															...
															onChange={(e) => handleToggleCheck(e, index)}
														/>
													}
													elementContent={
